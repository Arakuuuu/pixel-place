...
export default function Home() {
  ...
  async function buyPixel() {
    ...
    if (profile.coins < cost) { showToast('Not enough coins!'); setActionLoading(false); return }
    ...
    const { error: cErr } = await supabase
      ...
      .eq('id', profile.id)
    if (cErr) { showToast('Error: ' + cErr.message); setActionLoading(false); return }

    // Pay seller if resale
    if (selected.owner_id && selected.for_sale) {
      const { data: seller } = await supabase.from('profiles').select('coins').eq('id', selected.owner_id).single()
      if (seller) {
        await supabase.from('profiles').update({ coins: seller.coins + Math.floor(cost * 0.9) }).eq('id', selected.owner_id)
      }
      await supabase.from('transactions').insert({
        pixel_id: selected.id, seller_id: selected.owner_id, buyer_id: profile.id, price: cost,
      })
    }

    if (selected.id) {
      // Pixel row exists, update it
      await supabase.from('pixels').update({
        owner_id: profile.id, color, for_sale: false, sale_price: 0,
        price: Math.max(selected.price, cost), updated_at: new Date().toISOString(),
      }).eq('id', selected.id)
    } else {
      // New pixel, insert
      await supabase.from('pixels').insert({
        x: selected.x, y: selected.y, color, owner_id: profile.id, price: 1,
      })
    }
